...
namespace MAUIRESTAURANTE.Services
{
    public class APIService
    {
        ...
        {
            _baseUrl = "https://apiprotc.azurewebsites.net/";
        ...
        public async Task<double> CalcularTotal(int idOrden)
        {
            try
            {
                var response = await _httpClient.PutAsync($"/api/Factura/CalcularFactura/{idOrden}", null);
        ...
        public async Task<Plato> EditarPlato(Plato nPlato)
        {
            try
            {
                var content = new StringContent(JsonConvert.SerializeObject(nPlato), Encoding.UTF8, "application/json");
                var response = await _httpClient.PutAsync($"/api/Plato/IdPlato/{nPlato.IdPlato}", content);
        ...
        public async Task<bool> CrearPlatoOrden(PlatoOrden platoOrden)
        {
            try
            {
                var content = new StringContent(JsonConvert.SerializeObject(platoOrden), Encoding.UTF8, "application/json");
                var response = await _httpClient.PostAsync("/api/PlatoOrden/CrearPlatoOrden", content);
        ...
        public async Task<PlatoOrden> RestarCantidad(int idPlatoOrden)
        {
            try
            {
                var response = await _httpClient.PutAsync($"/api/PlatoOrden/RestarCantidad/{idPlatoOrden}", null);
                ...
                // Manejar el caso en que no se pudo restar la cantidad
                return null;
            }
            catch (Exception ex)
            {
                // Manejar cualquier excepción durante la solicitud
                Console.WriteLine($"Error al retsar cantidad al plato ordenado: {ex.Message}");
                return null;
            }
        }
